...
// Le fournisseur exige un numéro international continu pour le destinataire,
// mais un numéro inclus dans le texte est moins susceptible d'être filtré
// lorsqu'il est présenté comme un contact local lisible et espacé.
export function formatCustomerSmsDisplayPhone(phone) {
  const normalized = formatCustomerSmsPhone(phone);
...
export function makeCustomerSmsCarrierSafe(message, orderNumber = '') {
  let safeMessage = String(message || '');
  const normalizedOrderNumber = String(orderNumber || '').trim();

  if (normalizedOrderNumber) {
    safeMessage = safeMessage.replace(
      new RegExp(`\\s*#?${escapeRegExp(normalizedOrderNumber)}\\b`, 'gi'),
      '',
    );
  }

  // Protège également les anciens modèles enregistrés contenant CMD/ORD.
...
export function buildCustomerSmsMessage(eventCode, commande = {}, extra = {}, templateOverride = null) {
  ...
  const livreurTelephone = formatCustomerSmsDisplayPhone(
  ...
  const renderedMessage = template.replace(/\{([a-z_]+)\}/g, (match, variable) => (
    Object.prototype.hasOwnProperty.call(values, variable) ? values[variable] : match
  )).trim();

  return makeCustomerSmsCarrierSafe(renderedMessage, values.numero_commande);
}
