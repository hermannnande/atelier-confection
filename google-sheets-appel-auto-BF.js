// 🇧🇫 GOOGLE SHEETS BURKINA FASO -> PAGE "APPEL" (100% AUTOMATIQUE, SANS MENU)
...
// ⚠️ Ce script est dédié au pays Burkina Faso.
...
/**
 * Envoie UNE ligne vers l'API (route publique, sans JWT).
 */
function envoyerCommandeVersAPI(sheet, row, colMap) {
  ...
  // ⚠️ La route publique exige phone, name, taille, couleur, price.
  // Si l'un manque on saute la ligne (et on tagge l'erreur).
  if (!safeContact) {
    sheet.getRange(row, syncCol).setValue('❌ contact manquant');
    return;
  }
  
  // 📦 Format attendu par /api/commandes/public
  const commandeData = {
    token: API_TOKEN,
    country: COUNTRY_CODE,                       // pays_code dans la base
    client: safeNom,
    phone: safeContact,
...
function installerTriggersAutomatiques() {
  ...
  SpreadsheetApp.getUi().alert('✅ Installation Burkina Faso réussie !\n\nLes commandes seront automatiquement envoyées vers la page Appel (pays BF).\n\nAucun token JWT à gérer : ce script utilise la clé fixe partagée.');
}

function syncNow() {
  syncPending_();
  SpreadsheetApp.getUi().alert('Synchronisation Burkina Faso terminée !\nVérifie les logs (Affichage > Journaux).');
}
