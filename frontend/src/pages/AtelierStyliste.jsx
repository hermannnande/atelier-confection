...
import { Scissors, CheckCircle, AlertCircle, Package, RefreshCw, Eye } from 'lucide-react';
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in overflow-x-hidden max-w-full px-2 sm:px-4">
      ...
      {isReadOnly && (
        <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-3 sm:p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-blue-700">Vous consultez cette page en <strong>lecture seule</strong>. Seuls les stylistes peuvent marquer les tâches terminées.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
          <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-2 sm:p-3 lg:p-4 rounded-2xl shadow-lg flex-shrink-0">
            <Scissors className="text-white" size={24} strokeWidth={2.5} />
          </div>
          <div className="min-w-0 flex-1">
            <h1 className="text-xl sm:text-3xl lg:text-4xl font-black bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent truncate">
              Atelier - Styliste
            </h1>
            <p className="text-xs sm:text-sm lg:text-base text-gray-600 font-medium truncate">Découpe et préparation des modèles</p>
          </div>
        </div>
        ...
        <div className="flex flex-row sm:flex-col items-center sm:items-end gap-2 sm:gap-2 w-full sm:w-auto justify-between sm:justify-end">
          ...
          <div className="text-right flex-shrink-0">
            ...
            <p className="text-2xl sm:text-3xl lg:text-4xl font-black bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent">
      ...
      {commandesEnDecoupe.length === 0 ? (
        <div className="card text-center py-8 sm:py-12 lg:py-16 bg-gradient-to-br from-emerald-50 to-teal-50 max-w-full overflow-hidden">
          ...
          <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-2 px-4">✅ Aucune commande en attente de découpe</h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-3 max-w-full">
          {commandesEnDecoupe.map((commande) => (
            ...
            >
              ...
              <button
                onClick={() => !isReadOnly && handleEnvoyerCouture(commande._id)}
                disabled={isReadOnly}
                ...
                title={isReadOnly ? 'Action réservée aux stylistes' : 'Envoyer en couture'}
              >
                {isReadOnly ? (
                  <>
